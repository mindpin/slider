...
class Img4yeUploader
  constructor: (@options)->
    data = jQuery(@options["browse_button"]).data()
    @qiniu_domain   = data["domain"]
    @qiniu_basepath = QINIU_CONFIG["basepath"]
    @uptoken_url    = data["uptokenurl"]
...
jQuery(document).on 'ready page:load', ->
  if jQuery('.page-resource .btn-upload').length

    options =
      browse_button: jQuery('.page-resource .btn-upload')
      drag_area:    jQuery(document.body)
      paste_upload: true
    console.log options
